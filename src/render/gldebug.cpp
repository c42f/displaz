...
void _glError(const char *file, int line)
{
    ...
    {
        const std::map<GLenum, const char*> errorMap =
            {
        ...
        const auto errorIterator = errorMap.find(err);
        if (errorIterator != errorMap.end())
        {
            tfm::printfln("%s - %s:%i", errorIterator->second, file, line);
        }
